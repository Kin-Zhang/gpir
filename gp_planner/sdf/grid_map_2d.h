...
cv::Mat GridMap2D<T>::ImageSec() const {
  ...
  // since the origin is the left-bottom cornerd, filp around x-axis
...
//   for (int i = 0; i < cell_num_[0] - 10; i += 5) {
//     for (int j = 0; j < cell_num_[1] - 10; j += 5) {
//       const double xc = origin_[0] + i * cell_resolution_[0];
//       const double yc = origin_[1] + j * cell_resolution_[1];
//       Eigen::Vector2d grad;
//       auto dis = GetValueBilinear(Eigen::Vector2d(xc, yc), &grad);
//       x.emplace_back(xc);
//       y.emplace_back(yc);
