...
#include "gp_planner/benchmark/DL_IAPS/coarse_trajectory_generator/piecewise_jerk_speed_problem.h"
...
HybridAStar::HybridAStar() {
  ...
  step_size_ = 0.5;
  xy_grid_resolution_ = 0.3;

  delta_t_ = 1.0;
  traj_forward_penalty_ = 1.0;
  traj_back_penalty_ = 1e9;
  traj_gear_switch_penalty_ = 1e9;
  traj_steer_penalty_ = 0.0;

  traj_steer_change_penalty_ = 0.0;
}
...
bool HybridAStar::RSPCheck(
  ...
  std::shared_ptr<Node3d> node = std::shared_ptr<Node3d>(
      new Node3d(reeds_shepp_to_end->x, reeds_shepp_to_end->y,
                 reeds_shepp_to_end->phi, XYbounds_));
...
bool HybridAStar::ValidityCheck(std::shared_ptr<Node3d> node) {
  ...
  for (size_t i = check_start_index; i < node_step_size; ++i) {
    ...
    Box2d bounding_box = Node3d::GetBoundingBox(traversed_x[i], traversed_y[i],
                                                traversed_phi[i]);
...
std::shared_ptr<Node3d> HybridAStar::Next_node_generator(
  ...
  for (size_t i = 0; i < arc / step_size_; ++i) {
    ...
    const double next_phi = common::math::NormalizeAngle(
        last_phi + traveled_distance / 2.7 * std::tan(steering));
...
bool HybridAStar::GenerateSpeedAcceleration(HybridAStartResult* result) {
  ...
  for (size_t i = 0; i + 1 < x_size; ++i) {
    double discrete_steer =
        (result->phi[i + 1] - result->phi[i]) * 2.7 / step_size_;
    if (result->v[i] > 0.0) {
...
bool HybridAStar::GenerateSCurveSpeedAcceleration(HybridAStartResult* result) {
  ...
  if (total_t + delta_t >= delta_t * std::numeric_limits<size_t>::max()) {
    LOG(ERROR) << "Number of knots overflow. total_t: " << total_t
               << ", delta_t: " << delta_t;
    return false;
  }
  ...
  for (size_t i = 0; i + 1 < path_points_size; ++i) {
    double discrete_steer =
        (combined_result.phi[i + 1] - combined_result.phi[i]) * 2.7 /
...
bool HybridAStar::Plan(
  ...
  // LOG(INFO) << "map time " << Clock::NowInSeconds() - map_time;
  // load open set, pq
  ...
  auto astar_start_time = std::chrono::high_resolution_clock::now();
