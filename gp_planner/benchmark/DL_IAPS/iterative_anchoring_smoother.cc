...
#include "gp_planner/benchmark/DL_IAPS/math_utils.h"
...
bool IterativeAnchoringSmoother::Smooth(
  ...
  if (interpolated_size < 4) {
    ...
    DLOG(INFO) << "interpolated_warm_start_path smaller than 4, can't enforce "
                  "initial zero kappa";
  ...
  if (!SmoothPath(interpolated_warm_start_path, bounds, smoothed_path)) {
    return false;
  }
  ...
  DLOG(INFO) << "iterative anchoring path smoother time: "
             << path_smooth_diff.count() * 1000.0 << " ms.";

  const auto end_timestamp = std::chrono::system_clock::now();
  std::chrono::duration<double> diff = end_timestamp - start_timestamp;
  DLOG(INFO) << "iterative anchoring smoother total time: "
...
bool IterativeAnchoringSmoother::ReAnchoring(
  ...
  if (colliding_point_index.empty()) {
    DLOG(INFO) << "no point needs to be re-anchored";
    return true;
  }
  ...
  const size_t reanchoring_trails_num = static_cast<size_t>(50);
  const double reanchoring_pos_stddev = 0.25;
  const double reanchoring_length_stddev = 1.0;
...
bool IterativeAnchoringSmoother::SmoothPath(
  ...
  while (!is_collision_free) {
    ...
    DLOG(INFO) << "loop iteration number is " << counter;
    ++counter;
  }
  return true;
}
    ...
    for (const auto& obstacle_linesegments : obstacles_linesegments_vec_) {
      for (const LineSegment2d& linesegment : obstacle_linesegments) {
        if (ego_box.HasOverlap(linesegment)) {
          ...
          DLOG(INFO) << "point at " << i << "collied with LineSegment "
