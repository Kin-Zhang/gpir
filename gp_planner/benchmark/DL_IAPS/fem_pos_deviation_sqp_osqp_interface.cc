...
bool FemPosDeviationSqpOsqpInterface::Solve() {
  ...
  while (pen_itr < sqp_pen_max_iter_) {
    ...
    // DLOG(INFO) << "ctol is " << ctol << ", at pen itr " << pen_itr;

    if (ctol < sqp_ctol_) {
      DLOG(INFO) << "constraint satisfied at pen itr num " << pen_itr;
      DLOG(INFO) << "constraint voilation value drops to " << ctol
  ...
  DLOG(INFO) << "constraint not satisfied with total itr num " << pen_itr;
  DLOG(INFO) << "constraint voilation value drops to " << ctol
...
double FemPosDeviationSqpOsqpInterface::CalculateConstraintViolation(
  ...
  LOG(WARNING) << total_length << ", " << num_of_points_ << ", "
               << "curvature_constraint_sqr: " << curvature_constraint_sqr
  ...
  for (size_t i = 1; i < points.size() - 1; ++i) {
    ...
    double cviolation = curvature_constraint_sqr -
                        ...
                        (-2.0 * y_m + y_f + y_l) * (-2.0 * y_m + y_f + y_l);
    LOG(INFO) << "violantion: " << cviolation;
