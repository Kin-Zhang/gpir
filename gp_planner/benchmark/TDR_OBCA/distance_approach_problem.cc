...
bool DistanceApproachProblem::Solve(
  ...
  if (planner_open_space_config_.distance_approach_config()
    ...
    ptop = new DistanceApproachIPOPTInterface(
        ...
        planner_open_space_config_);
               ...
               .distance_approach_mode() == DISTANCE_APPROACH_IPOPT_RELAX_END) {
  ...
  if (status != Ipopt::Solve_Succeeded) {
    std::cout
        << "[INFO] *** Distance Approach problem error during initialization!"
        << std::endl;
    return false;
  }
  ...
  if (status == Ipopt::Solve_Succeeded ||
    ...
    */
    ...
    if (!failure_status.count(static_cast<size_t>(status))) {
      std::cout << "[INFO] Solver ends with unknown failure code: "
                << static_cast<int>(status) << std::endl;
    } else {
      std::cout << "[INFO] Solver failure case: "
                << failure_status[static_cast<size_t>(status)] << std::endl;
    }
  }
