...
int main(int argc, char** argv) {
  ...
  grid_map.FillConvexPoly(vector_Eigen<Eigen::Vector2d>{
      Eigen::Vector2d(40, 0.5), Eigen::Vector2d(45, 0.5),
  ...
  auto pose_fix_cost2 = noiseModel::Isotropic::Sigma(3, 0.01);
  const double qc = 0.1;
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (i == N - 1) graph.add(PriorFactor<Vector3>(key, xn, pose_fix_cost2));
    ...
    if (i > 0) {
      ...
      if (atof(argv[2]) > 0) {
        graph.add(GPKappaLimitFactor(key, 0.1, kappa, 0.0, 0.3, interval * i));
      }
      ...
      for (int j = 0; j < collision_check_num; ++j) {
        graph.add(GPInterpolateObstacleFactor(last_key, key, sdf, 0.01, epsilon,
                                              ...
                                              tau * (j + 1), kappa));
        if (atof(argv[2]) > 0) {
          graph.add(GPInterpolateKappaLimitFactor(last_key, key, 0.1, qc,
                                                  interval, tau * (j + 1),
                                                  kappa, 0.0, 0.3));
        }
      }
  ...
  for (int i = 0; i < x_coords.size(); ++i) {
    Key key = Symbol('x', i);
    init_values.insert<gtsam::Vector3>(key, init_res[i]);
  }
  ...
  cv::imshow("all", sdf->occupancy_map().BinaryImage());
  // cv::imshow("sdf", sdf->esdf().ImageSec());
  cv::waitKey(0);
