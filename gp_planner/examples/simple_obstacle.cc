...
int main(int argc, char** argv) {
  ...
  grid_map.FillConvexPoly(vector_Eigen<Eigen::Vector2d>{
      Eigen::Vector2d(20, 0), Eigen::Vector2d(35, 0), Eigen::Vector2d(35, 10),
  ...
  const int collision_check_num = 9;
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (i > 0) {
      ...
      //   graph.add(GPInterpolateObstacleFactor(last_key, key, sdf, 0.01,
      //   epsilon,
      //                                         interval * (i - 1), Qc,
      //                                         interval, tau * (j + 1)));
      // }
