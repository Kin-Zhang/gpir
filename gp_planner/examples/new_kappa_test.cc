...
// good example setup rosrun gp_planner new_kappa_test 2 0.18 0.23
// good example setup rosrun gp_planner new_kappa_test 3 0.15 0.2
...
int main(int argc, char** argv) {
  ...
  for (int i = 0; i < N; ++i) {
    ...
    if (i > 0) {
      ...
      graph2.add(GPKappaLimitFactor(key, 0.05, kappa, 0.0, atof(argv[3]),
                                    interval * i));
      ...
      for (int j = 0; j < collision_check_num; ++j) {
        ...
        graph2.add(GPInterpolateKappaLimitFactor(last_key, key, 0.05, qc,
                                                 interval, tau * (j + 1), kappa,
                                                 0.0, atof(argv[3])));
      }
    }
