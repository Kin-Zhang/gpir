...
double GPUtils::HingeKappaLimitLoss(
  ...
  // double error = 0.0, derivative = 0.0;
  // if (kappa_limit <= kappa && kappa < 1.5 * kappa_limit) {
  //   double delta = kappa - kappa_limit;
  //   error = delta * delta * delta;
  //   derivative = 3 * delta * delta;
  // } else if (1.5 * kappa_limit <= kappa) {
  //   double delta = kappa - kappa_limit;
  //   error =
  // }
