...
std::vector<WayPoint> Lane::way_points(const double start_s,
  ...
  // if (start_s > end_s) return waypoints;

  // auto lb = std::lower_bound(
  //     way_points_.cbegin(), way_points_.cend(), start_s,
  //     [](const WayPoint& p1, const double& val) { return p1.s <= val; });

  // if (lb == way_points_.end()) return waypoints;
  // if (lb != way_points_.begin()) {
  //   waypoints.emplace_back(Interpolate(*(lb - 1), *lb, start_s));
  // }

  // for (int i = lb - way_points_.begin(); i < points_num_; ++i) {
  //   if (way_points_[i].s <= end_s) {
  //     waypoints.emplace_back(way_points_[i]);
  //   } else {
  //     waypoints.emplace_back(
  //         Interpolate(way_points_[i - 1], way_points_[i], end_s));
  //   }
  // }
