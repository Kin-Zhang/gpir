...
TEST(Spline1dKernel, add_derivative_kernel_matrix_01) {
  ...
  for (int i = 0; i < kernel.gradient().rows(); ++i) {
    for (int j = 0; j < kernel.gradient().cols(); ++j) {
      EXPECT_DOUBLE_EQ(kernel.gradient()(i, j), ref_offset(i, j));
    }
  }
}
...
TEST(Spline1dKernel, add_derivative_kernel_matrix_02) {
  ...
  Eigen::MatrixXd ref_offset =
      Eigen::MatrixXd::Zero(kernel.gradient().rows(), 1);
...
TEST(Spline1dKernel, add_reference_line_kernel_01) {
  ...
  std::vector<double> ref_fx = {0.0};
  kernel.AddReferenceLineKernelMatrix(x_coord, ref_fx, 1.0);
  ...
  for (int i = 0; i < kernel.gradient().rows(); ++i) {
    EXPECT_DOUBLE_EQ(kernel.gradient()(i, 0), 0);
  }
}
...
TEST(Spline1dKernel, add_reference_line_kernel_02) {
  ...
  for (int i = 0; i < kernel.gradient().rows(); ++i) {
    if (i == 0) {
      EXPECT_DOUBLE_EQ(kernel.gradient()(i, 0), -6.0);
    } else {
      EXPECT_DOUBLE_EQ(kernel.gradient()(i, 0), 0);
    }
  }
...
TEST(Spline1dKernel, add_reference_line_kernel_03) {
  ...
  std::vector<double> ref_fx = {0.0, 2.0};
  kernel.AddReferenceLineKernelMatrix(x_coord, ref_fx, 1.0);
  ...
  for (int i = 0; i < kernel.gradient().rows(); ++i) {
    EXPECT_DOUBLE_EQ(kernel.gradient()(i, 0), ref_offset(i, 0));
  }
}
...
TEST(Spline1dKernel, add_reference_line_kernel_04) {
  ...
  for (int i = 0; i < kernel.gradient().rows(); ++i) {
    if (i < 6) {
      EXPECT_DOUBLE_EQ(kernel.gradient()(i, 0), 0.0);
    } else {
      EXPECT_DOUBLE_EQ(kernel.gradient()(i, 0), ref_offset(i % 6, 0));
    }
  }
}
}  // namespace common
